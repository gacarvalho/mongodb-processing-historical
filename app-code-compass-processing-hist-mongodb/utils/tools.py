...
def processing_old_new(spark: SparkSession, df: DataFrame):
    ...
    df_final = result_df.groupBy("new.id").agg(
        F.coalesce(F.first("new.customer_id"), F.first("old.customer_id")).alias("customer_id"),
        F.coalesce(F.first("new.cpf"), F.first("old.cpf")).alias("cpf"),
        F.coalesce(F.first("new.app"), F.first("old.app")).alias("app"),
        F.coalesce(F.first("new.rating"), F.first("old.rating")).alias("rating"),
        F.coalesce(F.first("new.timestamp"), F.first("old.timestamp")).alias("timestamp"),
        F.coalesce(F.first("new.comment"), F.first("old.comment")).alias("comment"),
        F.coalesce(F.first("new.app_version"), F.first("old.app_version")).alias("app_version"),
        F.coalesce(F.first("new.os_version"), F.first("old.os_version")).alias("os_version"),
        F.coalesce(F.first("new.os"), F.first("old.os")).alias("os"),
